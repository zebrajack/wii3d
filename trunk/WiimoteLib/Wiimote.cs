...
namespace WiimoteLib
{
	...
	public class Wiimote : IDisposable
	{
        ...
        /// but skip the first seq wii remotes
		/// </summary>
		public void Connect(int seq)
		{
			int index = 0;
			bool found = false;
			Guid guid;
				...
				if(HIDImports.SetupDiGetDeviceInterfaceDetail(hDevInfo, ref diData, ref diDetail, size, out size, IntPtr.Zero))
				{
					...
					if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
					{
						...
						if(attrib.VendorID == VID && attrib.ProductID == PID)
						{
                            ...
                            // read the calibration info from the controller, test if it's alive or not
                            bool alive = true;
                            if(!ReadCalibration())
                            {
                                ...
                                if (!ReadCalibration())
                                {
                                    //failed completely, this device may be a dead body
                                    alive = false;
                                }
                            }
                            ...
                            {
                                // we opened a dead body, close it
                                mStream.Close();
                                mHandle.Close();
                            }
						}
...
//					mWiimoteState.ClassicControllerState.AccelCalibrationInfo.MaxTriggerR = (byte)(buff[15] >> 3);
		...
		/// <summary>
		/// Read calibration information stored on Wiimote, return true(success) or false(fail)
		/// </summary>
		private bool ReadCalibration()
		{
			// this appears to change the report type to 0x31
			byte[] buff = null;
            bool success = TestRead(0x0016, 7, ref buff);
        ...
        public bool TestRead(int address, short size, ref byte[] buff)
        {
            ClearReport();
